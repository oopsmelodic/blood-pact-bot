...
- канал обмена — распознавание иконок/OCR и карточки разрола
...
| `/loot_scan screenshot duration` | Офицеры | Распознать до 5 предметов и открыть один общий разрол с изображением и кнопками 1–5 |
| `/loot_manual items duration` | Офицеры | Подготовить общий разрол максимум для 5 предметов |
...
- Скриншот сетки инвентаря распознаётся по игровым иконкам из Hero Siege Codex.
- Одинаковые предметы на одном скриншоте сохраняются как отдельные экземпляры для разрола.
- Общие или отсутствующие в атласе иконки бот не угадывает — их можно исправить вручную в предпросмотре.
- Перед запуском бот показывает офицеру список для проверки и ручного исправления.
- Один скриншот содержит максимум 5 предметов: они публикуются одним списком с кнопками 1–5 без кнопки «Пас».
- Если на скриншоте присутствуют названия, резервный OCR читает английский и русский текст.
- Каталог и атлас кэшируются на Railway Volume в `hero_siege_items.json` и `hero_siege_item_icons.webp`.
